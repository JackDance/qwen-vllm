import os 
from vllm import AsyncEngineArgs,AsyncLLMEngine # AsyncEngineArgs为vllm的异步传参库；AsyncLLMEngine为vllm的异步推理库
...
model_dir="/root/autodl-tmp/qwen-pretrained_model/qwen/Qwen-7B-Chat-Int4"
tensor_parallel_size=1 # GPU的数量
gpu_memory_utilization=0.9 # 允许vllm最多占用显卡的60%的显存
...
async def chat(request: Request):
    ...
    results_iter=engine.generate(prompt=None,sampling_params=sampling_params,prompt_token_ids=prompt_tokens,request_id=request_id) # 返回一个协程对象，其是一个可以等待的东西
    
    # 流式返回，即迭代transformer的每一步推理结果并反复返回
    if stream:
        async def streaming_resp():
            async for result in results_iter: # 异步等待，不会阻塞网络线程
                # 移除im_end,eos等系统停止词
